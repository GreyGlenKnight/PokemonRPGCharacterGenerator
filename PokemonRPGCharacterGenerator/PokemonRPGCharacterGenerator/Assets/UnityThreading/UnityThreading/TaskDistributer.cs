...
namespace UnityThreading
{
    public class TaskDistributor : DispatcherBase
	{
			...
			get
			{
				if (mainTaskDistributor == null)
					throw new InvalidOperationException(
                        "No default TaskDistributor found, " +
                        "please create a new TaskDistributor " +
                        "instance before calling this property.");
        ...
        /// with ProcessorCount x2 worker threads.
		...
		/// The task distributor will auto start his worker threads.
		/// </summary>
		/// <param name="workerThreadCount">The number of worker threads, 
        /// a value below one will create ProcessorCount x2 worker threads.</param>
		...
		protected override void CheckAccessLimitation()
		{
			...
			{
				throw new InvalidOperationException(
                    "Access to TaskDistributor prohibited when " +
                    "called from inside a TaskDistributor thread. " +
                    "Dont dispatch new Tasks through the same TaskDistributor. " +
                    "If you want to distribute new tasks create a new " +
                    "TaskDistributor and use the new created instance. " +
                    "Remember to dispose the new instance to prevent thread spamming.");
			}
		}
        ...
        internal override void TasksAdded()
        {
			if (MaxAdditionalWorkerThreads > 0 &&
                ...
                this.taskList.Count > workerThreads.Length))
            {
